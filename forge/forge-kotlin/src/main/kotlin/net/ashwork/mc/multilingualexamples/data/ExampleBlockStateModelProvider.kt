 ...
 */
class ExampleBlockStateModelProvider(output: PackOutput, efh: ExistingFileHelper): BlockStateProvider(output, ID, efh) {

    override fun registerStatesAndModels() {
        this.cubeAllItem(WAFFLE)
        this.blockWithItem(SQUISHED_WAFFLE) { name, block ->
            val waffle = WAFFLE.id.withPrefix("${ModelProvider.BLOCK_FOLDER}/")
            val squishedWaffle = name.withPrefix("${ModelProvider.BLOCK_FOLDER}/")
