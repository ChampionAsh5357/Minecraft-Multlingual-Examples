 ...
 */
class ExampleItemModelProvider(output: PackOutput, efh: ExistingFileHelper) extends ItemModelProvider(output, MultilingualExamples.ID, efh) {
    ...
    private def simpleItem(item: RegistryObject[_ <: Item], layer0: ResourceLocation): Unit =
        this.withExistingParent(item.getId.toString, "item/generated")
                .texture("layer0", layer0.withPrefix(s"${this.folder}/"))
}
